...
void Custom_cell::add_ecm_interaction( int index_ecm, int index_voxel )
{
	...
	if ( dens > EPSILON )
	{
		...
		if ( distance < dd )
		{
			...
			tmp_r *= dens * custom_data["cell_ecm_repulsion"];
		}

		// Cell adherence to ECM through integrins
		double max_interactive_distance = (custom_data["max_interaction_factor"]*phenotype.geometry.radius) + ecmrad;
...
void Custom_cell::degrade_ecm( double dt )
{
	...
	#pragma omp critical
	{
		...
		if ( dens > EPSILON )
		{
			dens -= (custom_data["ecm_degradation"] * pintegrin) * dt; // to change by a rate
			dens = dens > 0 ? dens : 0;
...
{ 
	if ( level == 0 )
		return contact_cell() > custom_data["contact_cell_cell_threshold"]; 
	else
		return contact_cell() > (2 * custom_data["contact_cell_cell_threshold"]); 
}
...
void Custom_cell::set_3D_random_motility( double dt )
{
    ...
    if ( probability < dt / custom_data["persistence"] )
    {
...
void Custom_cell::set_3D_polarized_motility( double dt )
{
    ...
    motility *= (1 - custom_data["polarity_coefficient"]);
    ...
    motility += custom_data["polarity_coefficient"] * pol_dir;
    // Normalized it
    normalize(motility);
...
void Custom_cell::set_motility( double dt )
{
    ...
    if ( freezed > 2 )
        return;
    switch( (int) custom_data["mode_motility"] )
    {
        case 0:
...
{
	// std::cout << "Instantiating custom cell" << std::endl;
	Custom_cell* pNew; 
...
bool Custom_cell::wait_for_nucleus_growth (Cell* cell, Phenotype& phenotype, double dt) {
	return relative_diff( 
		...
		pow(cell->custom_data["cell_radius"], 3.0) * 3.14159 * (4.0/3.0) * 2.0 
	) > UniformRandom() * 0.1;
}
